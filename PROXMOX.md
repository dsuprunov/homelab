...
export \
  ...
  MNT='/mnt/pve/user-storage' \
  STORAGE='user-storage'

#
# init
#
sgdisk -n 1:0:0 -t 1:8300 -c 1:user-storage "$DISK"
partx -u "$DISK"
udevadm settle
mkfs.ext4 -F -L user-storage "$PART"
...
pvesm status --storage "$STORAGE"; echo
grep -A3 "^dir: $STORAGE$" /etc/pve/storage.cfg; echo
findmnt "$MNT"
```
